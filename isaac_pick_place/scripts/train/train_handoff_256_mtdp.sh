...
# Train the HF/LeRobot MultiTask DiT policy on the 256px dual-arm handoff dataset.
#
# Example:
#   bash isaac_pick_place/scripts/train_handoff_256_mtdp.sh
#
# Common overrides:
#   STEPS=10000 BATCH_SIZE=8 GRAD_ACCUM_STEPS=8 bash isaac_pick_place/scripts/train_handoff_256_mtdp.sh
#   RUN_NAME=my_handoff_run bash isaac_pick_place/scripts/train_handoff_256_mtdp.sh
#   CHECKPOINT_PATH=/path/to/final_model bash isaac_pick_place/scripts/train_handoff_256_mtdp.sh
#   TENSORBOARD=0 bash isaac_pick_place/scripts/train_handoff_256_mtdp.sh
...
DATASET_DIR="${DATASET_DIR:-${PROJECT_ROOT}/experiments/lerobot_datasets/lerobot_handoff_handoff_100_joint_ee_3cam_v1}"
RUN_NAME="${RUN_NAME:-hf_mtdp_handoff_3cam_joint_ee_100success_$(date +%Y%m%d_%H%M%S)}"
...
STATE_MODE="${STATE_MODE:-handoff_joint_ee}"
