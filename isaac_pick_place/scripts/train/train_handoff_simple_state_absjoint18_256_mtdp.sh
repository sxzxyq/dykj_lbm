...
# Train Handoff V2 simplified-state policy with an 18D absolute post-action
# joint_pos target. This checkpoint is intended for offline/open-loop analysis;
# the current Isaac eval action space still expects 14D end-effector deltas.
...
DATASET_DIR="${DATASET_DIR:-${PROJECT_ROOT}/experiments/lerobot_datasets/lerobot_handoff_v2_simple_state_absjoint18_train${TRAIN_EPISODES}}"
VAL_DATASET_DIR="${VAL_DATASET_DIR:-${PROJECT_ROOT}/experiments/lerobot_datasets/lerobot_handoff_v2_simple_state_absjoint18_val${VAL_EPISODES}}"
RUN_NAME="${RUN_NAME:-hf_mtdp_handoff_v2_simple_state_absjoint18_${TRAIN_EPISODES}train${VAL_EPISODES}val_bs16acc4_50k}"
...
ACTION_REPRESENTATION="${ACTION_REPRESENTATION:-absolute_joint_pos}"
ACTION_LAYOUT="${ACTION_LAYOUT:-abs_joint_pos_18}"
...
EXPECTED_DATASET_VERSION="${EXPECTED_DATASET_VERSION:-handoff_v2_simple_state_absjoint18}"
